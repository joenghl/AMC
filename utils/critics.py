...
class AttentionCritic(nn.Module):
    ...
    def shared_parameters(self):
        ...
        """
        return chain(*[m.parameters() for m in self.shared_modules]) # *是解包参数，chain方便对不同集合中的每个参数进行操作，每个parameters()是一个集合
    ...
    def forward(self, inps, agents=None, return_q=True, return_all_q=False,
        ...
        sa_encodings = [encoder(inp) for encoder, inp in zip(self.critic_encoders, inps)] # (10, 1024, 128)
        # extract state encoding for each agent that we're returning Q for
        s_encodings = [self.state_encoders[a_i](states[a_i]) for a_i in agents] # (10, 1024, 128)
        ...
        for curr_head_keys, curr_head_values, curr_head_selectors in zip(
            ...
            # iterate over agents
            for i, a_i, selector in zip(range(len(agents)), agents, curr_head_selectors): # selector -> Q
                keys = [k for j, k in enumerate(curr_head_keys) if j != a_i] # K
                values = [v for j, v in enumerate(curr_head_values) if j != a_i] # V
                # calculate attention across agents
                attend_logits = torch.matmul(selector.view(selector.shape[0], 1, -1), # Q @ K^T
                                             torch.stack(keys).permute(1, 2, 0))
                # scale dot-products by size of key (from Attention is All You Need)
                scaled_attend_logits = attend_logits / np.sqrt(keys[0].shape[1]) # QK^T / sqrt(dk)
